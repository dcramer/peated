...
- Keep “Log a tasting” as the header's only record action. Offer “Record a bottle” only when a search miss or contribution context makes that action relevant.
...
- State an absence in words. Do not render an inactive shell without a useful next action.
- Offer a contribution action only when the member can supply the missing data.
- Explain zero search results, offer “Record this bottle,” and keep nearby matches visible.
...
- Reserve the repeating global loading animation for the root route boundary. Do not use it for hydration or a module-level wait.
- Keep errors inside the failed module. State what still works and offer a tonal retry action.
- Use the shared empty-state, loading-record-list, and module-error components for these contracts.
- Use a page-level state only when the route cannot render its normal content. Keep a module failure inside its module.
...
- Render page-level recovery states in the minimal error shell, not the application shell. A global 404 or root-layout failure owns its complete document and does not load session state, application providers, analytics, the header, or the footer.
...
adapter, and application frame together so a product page never mixes new
...
Use one plain canvas treatment for spacing and width. Render each component directly instead of wrapping it in a decorative preview card. If a component needs a surface, radius, minimum height, or shadow, the component owns it. Put realistic compositions under `Patterns`, not in a component's primitive states. The Storybook theme toolbar changes the complete story canvas.
