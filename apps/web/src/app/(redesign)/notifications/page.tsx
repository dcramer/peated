...
export default async function UnreadNotificationsPage(props: {
  searchParams: Promise<Record<string, string | string[] | undefined>>;
}) {
  if (!(await isLoggedIn())) {
    redirectToAuth({ pathname: "/notifications" });
  }

  const searchParams = await props.searchParams;
  const input = getApiQueryParams(searchParams, {
    numericFields: ["cursor", "limit"],
    overrides: { filter: "unread" },
  });
  ...
  return (
    <NotificationList
      emptyHeading="No unread notifications"
