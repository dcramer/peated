...
export const CatalogMigrationApplyResultSchema = z
  .object({
  ...
  .superRefine(({ postflightAudit, revision }, context) => {
    if (
      !sameCatalogMigrationDatabaseEvidence(
        postflightAudit.databaseEvidence,
        revision.databaseEvidence,
      )
    ) {
      context.addIssue({
        code: "custom",
        message:
          "postflight audit and revision database evidence must match exactly because they share one transaction",
        path: ["postflightAudit", "databaseEvidence"],
      });
    }
  });
