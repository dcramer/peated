...
export const RETAILER_LABEL_EXAMPLES: RetailerLabelExample[] = [
  ...
  },
  {
    ...
    notes: [
      ...
      "When the parent bottle identity is clear but the local candidate is over-specific, prefer a bottle-level create_new outcome instead of falsely matching the specific edition.",
    ],
  },
  {
    source: "Generic Retailer",
...
export function buildBottleClassifierInstructions({
  ...
  return [
    ...
    "Decide whether the reference matches the current bottle or release, another candidate target, a new bottle, a new release under an existing bottle, a new bottle plus release, or nothing in the candidate set.",
    "",
    "Available tools:",
    renderBulletLines([
    ...
    renderBulletLines([
      ...
      "Before returning `create_new`, use `openai_web_search` to validate the bottle traits that make the reference distinct unless local evidence is already decisive.",
      ...(hasBraveWebSearch
        ? [
    ...
    renderBulletLines([
      ...
      "Return `create_new` only when the reference clearly represents a bottle that is not already present in the candidate set.",
      ...
      "Do not return `no_match` when bottle identity is clear but the only local candidate is too specific. In that case prefer `create_new` for the parent bottle unless authoritative evidence validates the more specific candidate.",
