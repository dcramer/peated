...
export type PasskeyAuthenticationServices = {
  verifyChallenge: typeof verifyChallenge;
  verifyResponse: typeof verifyAuthenticationResponse;
};

const defaultServices: PasskeyAuthenticationServices = {
  verifyChallenge,
  verifyResponse: verifyAuthenticationResponse,
};

export function createPasskeyAuthenticateVerifyProcedure(
  services: PasskeyAuthenticationServices = defaultServices,
) {
  return procedure
    .use(authRateLimit)
    .route({
    ...
    .handler(async function ({ input, context, errors }) {
      ...
      try {
        await services.verifyChallenge(input.signedChallenge, challenge);
      } catch (err: any) {
        auditLog({
      ...
      let verification;
      try {
        verification = await services.verifyResponse({
          response,
...
export default createPasskeyAuthenticateVerifyProcedure();
