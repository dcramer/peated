...
import * as labelExtractor from "../../agents/labelExtractor";
import { routerClient } from "../router";

vi.mock("../../agents/labelExtractor");
...
describe("POST /labels/extract", () => {
  ...
  test("extracts from image URL", async () => {
    ...
    const result = await routerClient.labelExtract({
  ...
  test("requires either imageUrl or label", async () => {
    const err = await waitError(routerClient.labelExtract({}));
