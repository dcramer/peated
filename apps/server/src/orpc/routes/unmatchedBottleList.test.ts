...
describe("GET /bottles/unmatched", () => {
  test("requires authentication", async () => {
    const err = await waitError(() => routerClient.unmatchedBottleList());
  ...
  test("requires mod privileges", async ({ fixtures }) => {
    ...
    const err = await waitError(() =>
      routerClient.unmatchedBottleList({}, { context: { user } }),
    );
    expect(err).toMatchInlineSnapshot(`
  ...
  test("returns empty list when no aliases exist", async ({ fixtures }) => {
    ...
    const result = await routerClient.unmatchedBottleList(
      {},
      { context: { user } },
    );
  ...
  test("paginates results correctly", async ({ fixtures }) => {
    ...
    const firstPage = await routerClient.unmatchedBottleList(
      { limit: 2 },
      { context: { user } },
    );
    ...
    // Get second page
    const secondPage = await routerClient.unmatchedBottleList(
      { limit: 2, cursor: 2 },
      { context: { user } },
    );
