...
import {
  pickExistingDirtyParentAgeRepairTargetRelease,
...
type RepairRelease = DirtyParentAgeRepairRelease &
  Pick<BottleRelease, "description" | "imageUrl" | "tastingNotes">;

type ApplyDirtyParentAgeRepairResult = {
  aliasNames: string[];
  bottleId: number;
  releaseId: number;
};

export class DirtyParentAgeRepairBadRequestError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "DirtyParentAgeRepairBadRequestError";
  }
}
...
async function getBottleForRepair(tx: AnyTransaction, bottleId: number) {
  ...
  if (!bottle) {
    throw new DirtyParentAgeRepairBadRequestError("Bottle not found.");
  }

  if (bottle.statedAge === null || bottle.statedAge === undefined) {
    throw new DirtyParentAgeRepairBadRequestError(
      "Bottle does not contain a dirty parent age to repair.",
    );
  }

  if (bottle.numReleases <= 0) {
    throw new DirtyParentAgeRepairBadRequestError(
      "Bottle does not have child releases to justify moving the parent age down.",
    );
  }

  if (bottleMarketsStatedAge(bottle)) {
    throw new DirtyParentAgeRepairBadRequestError(
  ...
  if (hasBottleLevelReleaseTraits(bottle)) {
    throw new DirtyParentAgeRepairBadRequestError(
      "Bottle still contains non-age release traits on the parent record.",
    );
  }
...
function getConflictingAgeReleases(
  bottle: RepairBottle,
  releases: RepairRelease[],
) {
  return releases.filter(
    (release): release is RepairRelease & { statedAge: number } =>
...
async function backfillExistingReleaseMetadata(
  ...
  if (!updatedRelease) {
    throw new DirtyParentAgeRepairBadRequestError("Bottle release not found.");
  }

  return updatedRelease satisfies RepairRelease;
}

function resolveBottleLinkedNameReleaseId({
  defaultReleaseId,
  name,
  releases,
}: {
  ...
  name: string;
  releases: Array<
    Pick<RepairRelease, "fullName" | "id" | "name" | "statedAge">
  >;
}) {
  ...
  if (exactRelease) {
    return exactRelease.id;
  }

  const normalizedBottle = normalizeBottle({ name });
  if (normalizedBottle.statedAge !== null) {
    const ageMatches = releases.filter(
      (release) => release.statedAge === normalizedBottle.statedAge,
    );
    if (ageMatches.length === 1) {
      return ageMatches[0].id;
    }
  }
...
function resolveBottleAliasReleaseId({
  name,
  releases,
}: {
  name: string;
...
export async function applyDirtyParentAgeRepairInTransaction(
  tx: AnyTransaction,
  {
    bottleId,
    user,
  ...
  const parentStatedAge = bottle.statedAge;

  if (conflictingReleases.length === 0) {
    throw new DirtyParentAgeRepairBadRequestError(
      "Bottle does not have conflicting child release ages to repair.",
    );
  }
  ...
  if (existingTargetRelease) {
    ...
    try {
      ...
      if (err instanceof BottleReleaseAlreadyExistsError) {
        ...
        throw new DirtyParentAgeRepairBadRequestError(err.message);
      } else {
        throw err;
      }
    }
  ...
  for (const alias of bottleScopedAliases) {
    const releaseId = resolveBottleAliasReleaseId({
      name: alias.name,
...
export async function applyDirtyParentAgeRepair({
  bottleId,
  user,
}: {
  bottleId: number;
  ...
  const result = await db.transaction(async (tx) =>
    applyDirtyParentAgeRepairInTransaction(tx, {
      bottleId,
      user,
    }),
  );
