...
describe("resolveBottleReferenceTarget", () => {
  ...
  test("uses trusted exact aliases without calling the classifier", async ({
    fixtures,
  }) => {
    ...
    const result = await resolveBottleReferenceTarget({
      ...
      trustedAliasLookupNames: [bottle.fullName],
      user,
    });
  ...
  test("does not use normalized aliases as trusted fast-path matches", async ({
    fixtures,
  }) => {
    const user = await fixtures.User({ admin: true });
    ...
    const result = await resolveBottleReferenceTarget({
      ...
      trustedAliasLookupNames: ["Ardbeg 10 years old"],
      user,
    });
  ...
  test("does not use untrusted aliases as fast-path matches", async ({
    fixtures,
  }) => {
    const user = await fixtures.User({ admin: true });
    const bottle = await fixtures.Bottle({
    ...
    const result = await resolveBottleReferenceTarget({
      ...
      trustedAliasLookupNames: ["Ardbeg Ten Years"],
      user,
    });
