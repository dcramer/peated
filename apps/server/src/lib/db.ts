 ...
 * default to legacy trusted assertions, while unbound placeholders default to
 * generated and untrusted unless explicit provenance is supplied.
 */
export async function upsertBottleAlias(
  ...
  const { assignmentSource, assignmentTrusted, assignedById } = options;
  const hasTarget = bottleId !== null || releaseId !== null;
  const hasExplicitAssignmentOptions =
    assignmentSource !== undefined ||
    assignmentTrusted !== undefined ||
    "assignedById" in options;
  const nextAssignmentSource =
    assignmentSource ?? (hasTarget ? "legacy" : "generated");
  const nextAssignmentTrusted = assignmentTrusted ?? hasTarget;
  const nextAssignedById = assignedById ?? null;

  if (nextAssignmentSource === "generated" && nextAssignmentTrusted) {
    throw new Error("Generated bottle aliases cannot be trusted.");
  }
  ...
  const query =
    bottleId || releaseId
      ? await db.execute<BottleAlias>(
          sql`INSERT INTO ${bottleAliases} (bottle_id, release_id, name, assignment_source, assignment_trusted, assigned_by_id)
      VALUES (${bottleId}, ${releaseId}, ${name}, ${nextAssignmentSource}, ${nextAssignmentTrusted}, ${nextAssignedById})
      ...
      DO UPDATE SET
        ...
        assignment_trusted = CASE
          WHEN ${bottleAliases.bottleId} IS NULL OR (
            ...
            THEN EXCLUDED.assignment_trusted
            ELSE ${bottleAliases.assignmentTrusted}
          END,
        assigned_by_id = CASE
          WHEN ${bottleAliases.bottleId} IS NULL OR (
            ${hasExplicitAssignmentOptions}
