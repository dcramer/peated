...
| Mixed activity              | `CommunityFeed`                       | Author, action, date, grouped bottles, scores, and review links; uses larger photos in wide columns while keeping the missing-photo glyph small, the usual image size in sidebars and on mobile, and compact rows for library additions. |
