...
- Use Vitest for deterministic component logic such as state, validation,
  callbacks, and route loading decisions.
...
- Do not use automated tests to lock down visual markup: copy, decorative
  rendering, DOM structure, element counts, individual icons or labels, card
  composition, or pixel geometry.
...
- Test a loading fallback only when it changes behavior. Check its presentation
  during manual or agent-based visual QA.
