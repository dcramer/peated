...
remain valid. Web links, canonical metadata, and structured data use the current
slug.
...
Public tasting pages are discovered through links from public activity, member,
Bottle, and Entity pages. Peated does not include tasting URLs in catalog
sitemaps. Page access and privacy rules still determine whether a tasting can be
rendered or indexed.
...
Public member review pages are discovered through normal links. Peated does not
include them in catalog sitemaps.
