...
- [x] 1.1 Freeze a baseline: run the full classifier evals on the untouched prompt, commit intentional recordings, and capture a per-action result breakdown. (2026-07-06: 71/76; match/corrections/repair/image 100%; all 5 failures in new-bottles creates, including an Ardbeg Traigh Bhan abstention that violates the existing anti-abstain rule. 2026-08-09 refresh: [`audit/reference-classifier-baseline.md`](audit/reference-classifier-baseline.md) freezes the executable 88-case reference set and records the live result, confusion matrix, candidate recall, usage, timing, tool calls, and Suggested Change precision.)
...
- [x] 6.4 Add a per-action confusion breakdown to eval summaries that separates missed matches (false `no_match`) from false positives. (The frozen baseline report records the raw outcome matrix, false No Match count, and false-Match rate without adding a second eval metrics system.)
...
- [x] 7.1 Run `pnpm --filter @peated/bottle-classifier fixtures:validate`.
