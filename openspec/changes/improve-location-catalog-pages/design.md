...
Country and region previews use one shared `LocationPreviewCard`. Each card owns its location visual, name, bottle count, optional description, and truncation. Country visuals use country silhouettes. US region visuals use state silhouettes. Other region visuals use the country silhouette that provides their location context. Homepage and country overview grids own only their layout. The location route supplies country-scoped region data and the link to the complete Regions section.
