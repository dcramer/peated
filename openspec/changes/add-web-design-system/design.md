...
The rating model uses one 100-point ruler with band and point grains. `BandMark` accepts one band or point and uses a tick to preserve exact precision. `BandStack` accepts five band-pick counts and never converts them into a point. `Score` accepts a whole-number median, point count, and optional low and high points; it withholds the number below the product floor. `ScoreInput` emits a band, point, or null and reports grain changes to its caller. Published critic reviews remain attributed records on their native scales; only a permitted native 100-point score can join the caller-owned point pool. The scoped-search control combines a stationary scope trigger, an over-trigger menu, and a search input. The menu aligns its top and left outer edges with the control, aligns its option labels with the active scope label, lists the scopes directly without a redundant purpose label, and suppresses the query control's active ring while it is open. Pressing the existing query input closes the scope menu and gives query entry control without remounting the input or replacing caller-owned behavior. Query, scope, result, and permission state remain owned by the caller. These contracts preserve product meaning without creating a generic style API.
...
The tasting rating input uses the five canonical bands and whole-number points on one ruler. A band is never converted into a point. Grain belongs to each tasting, while the caller can remember the last grain as the next default. Colour input uses the existing 21-step scale. Picture input delegates native files to its caller. The tasting-form pattern composes these controls without adding fields that the tasting schema does not own.
...
The tasting-form pattern keeps one component tree across desktop and mobile viewports. Storybook gives the band and point states separate named stories. CSS reflows the rating input and the existing date, serving style, colour, notes, and picture controls without replacing them with a separate compact workflow.
...
### Use one rating ruler at two grains

Pass/Sip/Savor and the account-level Simple/Advanced choice are superseded. Members pick one of five trade-aligned bands or type an exact point for each tasting. Exact member points and permitted native 100-point publication scores share one point pool, summarized by a whole-number median. Band picks remain ranges and have their own five-bin distribution. Frontend components implement this contract before product adapters cut over to the separate API change.
