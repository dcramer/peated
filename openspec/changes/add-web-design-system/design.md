...
Links own visible interaction beyond the browser cursor. Neutral text links move from ink to the deeper accent and gain a 1px underline on hover, then use accent while pressed. Linked surface cards and rows step from surface to inset on hover and to accent tint while pressed without moving or gaining elevation. A destination card, record row, bottle identity row, or semantic table row with a primary destination uses that link across its surface by default, including when it also contains a secondary link or control. Those nested controls remain independent targets above the primary link. Cards and rows without a primary destination do not show a linked hover treatment. Plain linked rows use 12px horizontal padding and an equal negative margin so their interaction surface extends past the text grid while their content stays aligned. Data tracks stay visible against each surface tone. Keyboard focus uses the shared 2px inset ring on the complete actionable surface. Header navigation keeps database destinations available on every page. Its current destination uses ink-colored display type at weight 700 with `aria-current="page"`; the accent rule remains reserved for page tabs.

The rating model separates tasting bands from review scores. `BandMark` accepts one tasting band. `BandStack` accepts five tasting-band counts and never converts them into a point. `Score` accepts a whole-number review median, member and external score counts, and optional low and high scores; it shows every counted score. Published critic reviews remain attributed records. The interface shows a critic number only when its permitted native score already uses the 100-point scale and never converts another scale. The scoped-search control combines a stationary scope trigger, an over-trigger menu, and a search input. The menu aligns its top and left outer edges with the control, aligns its option labels with the active scope label, lists the scopes directly without a redundant purpose label, and suppresses the query control's active ring while it is open. Pressing the existing query input closes the scope menu and gives query entry control without remounting the input or replacing caller-owned behavior. Query, scope, result, and permission state remain owned by the caller. These contracts preserve product meaning without creating a generic style API.
...
The dedicated search route is a focused utility screen. It centers a compact route heading and the shared page search without the surface banner used by catalog landing pages. It does not force focus on load, so phone browsers keep the heading and surrounding context visible until the member chooses to enter a query. Bottle-selection intents reuse the same route structure and change only the contextual title, allowed scope, result destinations, and contribution handoff.
...
The signed-out homepage fetches public stats, reviews, countries, Scottish regions, distilleries, recent bottles, and the median-score ranking through the anonymous server client and hydrates the same React Query keys used by its client components. Highest rated uses the bottle list's `-score` order and `minScore` presence filter, so the route does not derive a ranking. The origin directory shows the three largest API-owned country records as map tiles, aggregates the remaining returned records into one truthful remainder tile, and lists the four largest supplied Scottish regions below them. The route remains request-time rendered because the application shell varies by session. Signed-in activity and member data remain client-owned. Both homepage states use the page ground for the application header so the shell reads as one surface; inner routes keep the separate header surface. The signed-out root header shows database navigation and omits its global search because the hero owns the page search. Popularity, annual rankings, critic-source totals, and highest-rated bottles scoped to one distillery wait for the server-owned discovery contract tracked in GitHub issue #781.

The signed-out homepage introduction describes the records a visitor can browse without claiming complete catalog coverage or depending on a large production count. Live totals remain appropriate in factual search, directory, and statistics modules where they identify the records that currently exist.

The entity detail route family uses one nested route frame for the entity header and tabs. Overview, bottles, tastings, and the optional SMWS codes reference page replace only the selected section. The entity-details response supplies identity, kind, location, ownership, core facts, and bottle totals. Kind is its only entity classification. Header actions sit below the identity copy and align with its left edge. Small route-local client components handle live queries, URL state, and actions that require browser hooks. Contextual bottle creation maps brand, bottler, and distillery kinds to their matching explicit Bottle field; blender and company records do not infer one. Brand, bottler, distillery, and blender records keep their Bottles or Bottlings section visible when it is empty and offer an Add a bottle button. The bottle-list response supplies origin, age, ABV, median review score, tasting-band counts, sorting, and cursor pagination. Entity overviews, entity bottle tabs, the bottle catalog, and homepage bottle modules use the same BottleList and BottleIdentityRow components. A shared helper maps each Bottle to the standard identity, metadata, ratings, member state, and related-release props. The overview shows those rows directly under its Bottles or Bottlings section without a second list title or a generic catalog summary. The tasting-list response supplies tasting entries and cursor pagination. The SMWS reference page renders the fixed code registry and enriches matching distillers with the existing SMWS endpoint. Company portfolio sections wait for the current-owner list filter tracked in GitHub issue #772 instead of treating bottle relationships as ownership. Storybook documents the reusable entity header, bottle list, tasting entries, and sparse states instead of copying complete routes. The live route omits operating status, still count, capacity, entity-level community measures, and history until an owned API contract supplies those values.
...
The final component pass removes compatibility facades and Storybook-only copies after their consumers move. Shared page layout helpers stay server-compatible. `ItemList`, `ItemRow`, `Avatar`, and `TextLink` use plain UI nouns. Pickers share private listbox navigation behavior while keeping their different product contracts and visuals.
