...
The signed-out homepage fetches public stats, reviews, regions, distilleries, and recent bottles through the anonymous server client and hydrates the same React Query keys used by its client components. The route remains request-time rendered because the application shell varies by session. Signed-in activity and member data remain client-owned. The signed-out root header shows database navigation and omits its global search because the hero owns the page search. Popularity, annual rankings, and highest-rated distillery bottles wait for the server-owned discovery contract tracked in GitHub issue #781.
