...
- [x] 8.1 Close the preflight task as historical after the approved production
      migration completed; do not rerun a migration-only audit against the
      post-cutover schema. The original task was to run and retain a fresh
      read-only audit from the exact candidate Git and database revisions,
      reconcile every count, and obtain explicit approval.
- [x] 8.2 Close the backup gate as historical after the approved one-shot
      migration; no repository artifact can establish or recreate that backup.
- [x] 8.3 Close the writer-shutdown gate as historical after direct-Bottle
      uniqueness was activated and BottleRelease writes were removed. The gate
      required old application and worker processes not to write legacy-only
...
- [x] 8.5 Validate counts, mappings, direct Bottle references, aggregates,
      ...
      before accepting new catalog-consumer traffic. Direct-Bottle uniqueness
      was activated after the production migration completed.
...
- [x] 8.10 Defer physical BottleRelease deletion to a separate, explicitly
      approved change with its own backup. Retained legacy storage is not part
      of the application runtime and must not be removed as archival cleanup.
- [x] 8.11 Defer the zero-legacy audit and full repository gate to that future
      physical-cleanup change.
