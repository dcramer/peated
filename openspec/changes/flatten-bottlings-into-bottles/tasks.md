...
- [ ] 5.4b Convert BottleRelease update into an instrumented adapter that requires a completed promotion mapping, applies an exact-only canonical Bottle patch without mirroring the legacy release row, and returns the exact CatalogTarget replacement.
- [ ] 5.4c Define and implement canonical concrete Bottle deletion with permanent promotion-mapping, group membership, representative, target, and tombstone semantics, then convert BottleRelease delete into an instrumented adapter over that operation.
