...
- [x] 5.8 Collapse classifier application and retained repair/create-release inputs into one independently complete concrete Bottle plus its exact target; create or reuse groups automatically from singleton or validated trusted-source context, never mutate a parent or write/finalize BottleRelease, and retain the original classifier action and structured source evidence while emitting `create_bottle` or `match_existing` decisions.
