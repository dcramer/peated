...
- [ ] 6.11 Add batch checkpoints, progress metrics, retry-safe errors, bounded transactions, and dry-run mode to the backfill command; sequence dependent alias/observation backfill only after its parent family's core promotion completes and exclude task 6.5a's canonical alias plus every other overlap; retained reports identify the exact candidate Git and database migration revisions in addition to the generated timestamp and database name.
