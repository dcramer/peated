...
- [ ] 5.6c Update direct user/API Review creation and mutation paths that have exact or generic intent to dual-write a validated `targetId`; exclude shared alias propagation owned by task 5.6b and unresolved classifier-created reviews deferred to tasks 5.8/5.9.
- [ ] 5.6d Update direct collection mutations to dual-write a validated exact or generic `targetId`, preserving target-aware set-membership conflict handling and leaving reads and legacy backfill to tasks 7.3 and 6.
- [ ] 5.6e Update direct flight mutations to dual-write a validated exact or generic `targetId`, preserving target-aware set-membership conflict handling and leaving reads and legacy backfill to tasks 7.3 and 6.
