...
class Follow(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["from_user_id"], ["user.id"], name="follow_from_user_id_user_id_fk"
        ),
        ForeignKeyConstraint(
            ["to_user_id"], ["user.id"], name="follow_to_user_id_user_id_fk"
        ),
    ...
    from_user = relationship(
        "User", foreign_keys=[from_user_id], back_populates="follow"
    )
