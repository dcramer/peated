...
class Entity(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["created_by_id"], ["user.id"], name="entity_created_by_id_user_id_fk"
        ),
    ...
    bottle = relationship(
        "Bottle", foreign_keys="[Bottle.bottler_id]", back_populates="bottler"
    )
    bottle_ = relationship(
        "Bottle", foreign_keys="[Bottle.brand_id]", back_populates="brand"
    )
    bottle1 = relationship(
        "Bottle", secondary="bottle_distiller", back_populates="distiller"
    )
