...
| [C15: Composed SMWS cask code](./C15-composed-smws-cask-code.md)                 | Preserve a verified SMWS code composed from separate label fields                    | Accepted: fixed the regression with no added model work                        |
| [C16: Raw photo-label text](./C16-preserve-raw-photo-label.md)                   | Preserve the extractor's raw label transcription in classifier evidence              | Accepted: repaired evidence loss with no model work                            |
| [C17: Image batch and lot role](./C17-image-batch-lot-role.md)                   | Keep package codes out of `edition` unless the label presents a marketed variant     | Rejected: extraction improved, but classification did not                      |
