...
const BOTTLE_OPERATION_POLICY = [
  ...
  renderBulletLines([
    ...
    "One label-image extraction can fill a missing scalar Bottle fact, but it cannot replace a populated age, ABV, year, cask, outturn, or similar factual value by itself. Such a replacement needs a matching structured Bottle observation or two agreeing label images; an unstructured web result may inform review but does not prove the factual value. A canonical name or edition wording correction can use one readable label plus the matching producer product title.",
