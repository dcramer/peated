...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "Every marketed release is one independently complete Bottle; BottleGroup assignment is automatic downstream. Edition, batch, chapter/volume, year, ABV, cask strength, single-cask, recipe, pick, and barrel traits identify that Bottle, never a parent or child object.",
    "A marker in the sold name — listing title, product-page name, or marketed label display name — is exact Bottle identity by default. Distinct SKUs, per-batch specifications, and sibling marker rows support that classification but are not required. A code printed separately, including a two-digit-year alphanumeric run code, is a lot observation unless evidence shows the product is sold by it. Cask/barrel numbers on single-cask products are exact-cask identity, not lot codes.",
    "For a photo with a plain expression, a separate year-prefixed `Batch No.` code, and both plain and code-specific candidates, contrastive web search is mandatory; neither OCR order nor the code-specific row proves marketed identity. If reviewed evidence shows one SKU across codes, match the plain Bottle and keep the code in `observation.selector` (for example `Batch No. 23J12`). The code-specific row's exact edition match is catalog evidence, not authority.",
    ...
    "Match marketed markers at full precision: `Act 12` does not cover `Act 12 Scene 9`, and a broader batch, chapter, volume, or vintage cannot cover a more specific sold-name marker. When the full marker is supported and absent locally, create the complete Bottle rather than match the partial marker.",
    ...
    "Keep every supported expression, finish, variant, edition, year, age, ABV, canonical cask type/size/fill, and cask flag on `proposedBottle`; a web mention alone is insufficient. Do not invent a parent/group to hold those facts.",
  ...
  renderBulletLines([
    ...
    "Structured extraction is strong evidence, but one isolated numeric conflict can be noise when the raw name and extracted expression state the same value, an exact alias/candidate agrees, and other direct traits corroborate it. For example, several direct `10-year-old` signals can outweigh one extracted `stated_age = 12`; match and explain the discarded conflict. Do not use this exception when the reference is ambiguous, candidates disagree, or corroboration is missing.",
  ...
  renderBulletLines([
    ...
    "2. Apply the sold-name versus lot-code rules above before choosing identity. `Batch` wording alone does not market a separate year-prefixed run code. With plain and code-specific candidates, search contrastively; the exact local row is evidence, not authority. Keep marketed markers on the Bottle and lot codes in `observation`.",
    ...
    "4. Creation requires reviewed evidence that establishes the identity and create-critical traits; source text, label/image, local catalog, closed-form identifiers, or web evidence may provide it. Use web search only to fill gaps, and rerun local search when web evidence reveals a new decisive trait.",
