...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "`repair_bottle`: choose this when an existing bottle is the right identity but its bottle-level canonical fields need correction. Return `matchedBottleId` and the repaired `proposedBottle`. Do not choose `repair_bottle` only to fill missing optional facts such as ABV on an otherwise exact match; use `match` unless the stored identity conflicts with evidence.",
