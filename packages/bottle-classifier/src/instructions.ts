...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  "Core Rules:",
  renderBulletLines([
    "Bottle-versus-release judgement is model-led. Use marketed identity, sibling evidence, and whisky-community meaning.",
    "False positive matches are worse than review or `no_match`.",
    ...
    "Judge web-result quality from the result content, source independence, specificity, and corroboration. Do not blindly trust retailer SEO, copied snippets, or a source just because its domain looks familiar.",
    ...
    "A missing house category is not by itself a reason for `no_match`: if reliable evidence confirms a real whisky product but no category enum fits, create the bottle with `category = null`.",
    "Missing optional fields such as category, ABV, cask details, or age are not creation blockers when the stable bottle identity is supported; leave unsupported fields null and record the uncertainty.",
    "After web confirms an omitted canonical trait, re-run `search_bottles` with that trait before returning a create action.",
    "If reliable web evidence supports a real bottle identity and refreshed local search finds no safe candidate, return the appropriate create action; do not use `no_match` just because the bottle is absent locally.",
    "`no_match` must not be paired with `confidenceBasis.webEvidence = supportive` or `confidenceBasis.band = auto_verification`; those signals mean you have enough evidence for a match or create action unless a concrete contradiction is present.",
    "If web evidence contradicts the source wording as a standalone product or maps it to a different canonical program, do not create the source wording as canonical.",
    "When evidence maps source wording to a different canonical program, either use that evidenced program identity exactly or return `no_match`; do not create a hybrid source-title plus inferred series.",
    "Do not create a broader base bottle as a fallback when evidence points to a different unresolved program or release.",
  ]),
  "",
  "Schema:",
  renderBulletLines([
    ...
    "`category`: house value or `null`; do not use fallback buckets. Real whisky products may still be created with `category = null` when the style is outside the enum.",
  ]),
  "",
  ...
  renderBulletLines([
    ...
    "`familyContext` is retrieved evidence, not a rule.",
    ...
    "Generic words such as `Aged`, package text, condition text, retailer SEO, volume, and gift packaging are not identity.",
  ...
  "",
  "Bottle/Release Boundary:",
  renderBulletLines([
    "A parent bottle is the stable marketed product family.",
    "A release is reusable bottling-level identity: vintage, release year, batch, edition, release-specific ABV or age, cask strength, single cask, cask type, cask size, or cask fill.",
    "Generic strength wording alone is not enough to create a new canonical bottle or child release; look for batch, ABV, year, cask, selector, or reliable web evidence that establishes a marketed family.",
    "If a parent has child releases, compare extracted release traits before matching only the parent or creating another release.",
    "When stable family is clear and annual year or numbered child marker varies, do not leave that differentiator stranded on `proposedBottle`; put it in `proposedRelease`.",
    "When an existing parent bottle already covers the stable family and the source adds age, vintage, release year, batch, edition, or cask details, prefer `create_release` under that parent over `create_bottle` unless evidence shows those traits are stable parent identity.",
    "Do not create a newly inferred program parent from a sparse shorthand when reliable evidence says the program needs missing selector, recipe, barrel, pick, or release context; use `no_match` unless a reusable parent is already a safe local candidate.",
    "Do not treat a year-first title as bottle-level year identity by default unless the source says vintage, distilled, or distillation.",
    "If bottle identity is clear but release identity is not, use the bottle-level outcome.",
    "If the only local candidate is more specific than the source supports, treat it as over-specific unless web evidence validates the missing differentiating traits.",
  ]),
  "",
  "Exact-Cask:",
  renderBulletLines([
    "Use `identityScope = exact_cask` only when the exact cask itself is marketed bottle identity.",
    "SMWS bottle code is an exact-cask identity anchor; subtitle differences can be observation-level.",
    "For non-SMWS references, cask or barrel wording in a title or snippet is not deterministic proof of exact-cask identity. Use exact_cask only when reliable evidence says the listed product itself is the exact single cask, not merely when a cask number appears as a detail.",
    "Exact-cask identity does not create child releases.",
  ]),
  "",
  "Confidence:",
  renderBulletLines([
    "Confidence is earned from evidence. Fill `confidenceBasis`.",
    "`confidenceBasis.band = auto_verification` requires concrete positive evidence and no unresolved material risk.",
    "If you include any `confidenceBasis.unresolvedRisks`, do not use `confidenceBasis.band = auto_verification`.",
    "Use `confidenceBasis.band = current_assignment` when cleanly reaffirming the current bottle/release assignment.",
    "Use `confidenceBasis.unresolvedRisks` for candidate conflicts, sibling ambiguity, weak web evidence, and release/bottle boundary uncertainty.",
    "In `confidenceBasis.positiveEvidence`, name why each decisive source is reliable enough for the decision, such as official producer page, independent whisky database, competition record, critic review, or corroborated retailer listing.",
    "Use `openai_web_search` or `brave_web_search` when one concrete missing or disputed trait controls confidence.",
    "Use `search_bottles` after web evidence confirms a trait that could recover a better local candidate or release.",
