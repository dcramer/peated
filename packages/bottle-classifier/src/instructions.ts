...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  "Task: classify one whisky reference as a complete Bottle against Bottle rows and retained legacy bottling candidates.",
  ...
  "Input Map:",
  renderBulletLines([
    "Candidates can be Bottle rows or retained legacy bottling targets. Use `kind` and `releaseId`; legacy shape is match evidence only.",
    "`familyContext` is transitional evidence about sibling Bottles and legacy child bottlings; it is not a deterministic rule and cannot select a BottleGroup.",
  ]),
  "",
  ...
  renderBulletLines([
    ...
    "After verifying a common brand and stable core expression, multiple legacy rows that differ by release year or edition are evidence about the observed exact Bottle, not authority to choose a BottleGroup or repair a parent.",
    ...
    "Existing child releases/bottlings are legacy catalog evidence. They may be matched when exact, but they do not authorize parent repair, parent creation, or group selection.",
    "Keep every supported expression, finish, variant, edition, year, age, ABV, and cask trait on the complete `proposedBottle`; do not add facts merely because web results mention them.",
  ...
  renderBulletLines([
    ...
    "Name the decisive evidence and material risks, especially candidate conflicts. A retained legacy release candidate is relevant only when it could be the exact match; do not treat future BottleGroup modeling as an unresolved risk.",
    ...
    "For `create_bottle`, carry over every source-supported structured exact field from extraction, including `edition`, `releaseYear`, `vintageYear`, `statedAge`, ABV, and cask flags unless the rationale explains why a field is observation-only.",
