...
const BOTTLE_IDENTITY_POLICY = [
  ...
  renderBulletLines([
    ...
    "Keep the consumer-facing Brand, producing distilleries, and market-facing bottler distinct even when one Entity fills multiple roles. Role distinctness does not require distinct Entities or a separately named imprint. `bottler` exists only when product-specific marketing establishes an independently meaningful, market-facing bottling role or release imprint. An ordinary official Brand or distillery bottling has no bottler; ownership, importing, distribution, packing, or page hosting does not establish one.",
...
const BOTTLE_OPERATION_POLICY = [
  ...
  renderBulletLines([
    ...
    "Clearing a populated Brand, bottler, distillery, series, category, or shared age is a correction. Propose it only when positive product-specific evidence contradicts the stored relationship or value. Source omission, role equality, and lack of a separately named imprint are not contradictions.",
    "Changing a populated exact field requires evidence tied to the same exact marketed release. A value from another batch, edition, year, or exact cask establishes variability, not a correction to the inspected Bottle.",
    "When product-specific evidence establishes that an expression's batches vary in numeric ABV but are consistently marketed as cask strength, barrel proof, or barrel strength, preserve the exact ABV boundary while using the stable strength trait. Numeric variability is not evidence that the boolean strength trait varies.",
