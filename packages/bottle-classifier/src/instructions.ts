...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "Do not match an edition-specific candidate to a broader source that omits that edition marker; create or repair the clean parent when evidence supports the broader bottle.",
    ...
    "Exact local bottle rows can be dirty legacy bottlings. Exact-name match is not enough when the row carries bottling-specific traits that should be split under a stable parent.",
    "However, when the source includes the same concrete bottling marker and the exact local bottle row has no conflicting canonical traits, treat that existing row as already covering the observed identity. Do not create a duplicate release solely to remodel a legacy bottle row under a clean parent.",
    "This existing-target rule overrides clean-parent modeling: if matching the exact candidate avoids duplicate catalog creation, match it and leave parent/release cleanup to downstream catalog repair.",
    ...
    "Do not match a candidate whose name contains a cask/barrel code, bottle number, outturn, or selector that the source lacks when evidence also supports an uncoded product identity; absence of a cleaner local row means create the supported uncoded product, not match the narrower coded row.",
  ...
  "Decision Workflow:",
  renderBulletLines([
    "Resolve source identity before catalog outcome: identify the bottle family plus exact release/bottling details, then decide whether that exact target already exists or needs creation.",
    "Use local Peated candidates like prior-art evidence: they show existing targets and modeling patterns, but they must not collapse a clear source bottling into a broader or wrong nearby row.",
    "If an existing local bottle or release already covers the exact observed bottle/release identity, choose `match` for that target instead of creating a duplicate cleaner-modeled child release. Catalog remodeling can happen later; the classifier outcome should answer whether the observed target already exists.",
    "Do not choose `create_release` solely to remodel an existing exact bottle candidate under a cleaner parent. If the exact candidate has the same concrete edition, batch, ABV, or other bottling marker and no conflicting canonical traits, return `match` for that existing target.",
    "Use local candidates first; use web search for disputed, missing, or create-critical traits. When a finish, expression, or variant separates close candidates, search contrastively for the source wording and the plainer candidate identity. Prefer broad unquoted product-word queries over exact quoted retailer titles.",
    "Creation requires supportive web evidence and a local candidate check that covers decisive traits; rerun local search when web evidence reveals a decisive trait not already covered by provided candidates.",
    "Decide bottle-vs-bottling scope before trusting exact-name candidates. Exact local rows are unsafe when they appear to be legacy bottling rows under a stable family.",
    "Match when an existing bottle or release/bottling covers the marketed identity without conflicting canonical traits. Before matching an exact bottle row, check whether bottling-level traits in that row should instead be a child release under a stable family.",
    "Repair only when the current/local target identity is right but stored canonical fields make that target identity unsafe. Missing optional facts or cleanup opportunities are downstream enrichment; do not let them block match/create.",
    "Create the narrowest supported target: bottle, bottling under an existing clean parent, or bottle plus bottling.",
  ...
  "Action Semantics:",
  renderBulletLines([
    "For bottling markers under a stable family: use `create_release` with a clean parent, `repair_parent_and_create_release` when the selected parent stores conflicting bottling traits, and `create_bottle_and_release` only when no existing candidate can serve as that parent.",
    "When no local parent exists for a stable family plus bottling marker, use `create_bottle_and_release`; do not collapse the marker into `proposedBottle` just because there is no parent to attach to yet.",
    ...
    "`create_bottle_and_release` is for a missing family parent, not for a dirty same-family row; repair the dirty row when it can become the proposed parent by removing bottling traits.",
    "Choose the parent by cleaned family identity, not by highest score alone: use a clean parent when present, otherwise use a dirty same-family row as the repair parent when removing release markers yields the proposed parent.",
    "Do not use `repair_parent_and_create_release` when a clean parent candidate already exists. Dirty sibling rows prove release modeling; they do not make the clean parent dirty.",
  ]),
  "",
  "Output Contract:",
  renderBulletLines([
    ...
    "`auto_verification` requires concrete positive evidence and no unresolved material risk. Only list risks that could change the action or target; missing optional ABV, distillery, producer-controlled source evidence, minor equivalent name wording, or hypothetical future siblings are not material when they are not needed to distinguish the target.",
    ...
    "`match`: choose this when an existing bottle or release/bottling already safely covers the marketed identity. Return `matchedBottleId` and, for release matches, `matchedReleaseId`.",
    "`repair_bottle`: choose this when an existing bottle is the right identity but its stored canonical fields make that identity unsafe. Return `matchedBottleId` and the repaired `proposedBottle`. Do not choose `repair_bottle` only to fill missing optional facts such as ABV or to remove questionable non-target-defining metadata; use `match` and leave enrichment/cleanup to downstream repair work.",
    "`create_bottle`: choose this when the source supports a new standalone bottle and no reusable child bottling is needed. Return `proposedBottle` only.",
    "`create_release`: choose this when a clean existing parent bottle should receive a new child bottling. Return that clean parent as `parentBottleId`, not a dirty/exact child-like row, plus `proposedRelease`.",
    "`create_bottle_and_release`: choose this when the source supports both a missing stable parent bottle and a child bottling under it. Return both `proposedBottle` and `proposedRelease`.",
    "`repair_parent_and_create_release`: choose this only when no clean parent candidate exists and an existing candidate can become the family parent by removing bottling-specific traits. Return `parentBottleId`, repaired parent `proposedBottle`, and `proposedRelease`.",
    "`no_match`: choose this when there is no safe existing target and no supported create action, or when creating would invent an ambiguous hybrid.",
