...
const BOTTLE_IDENTITY_AND_EVIDENCE_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "For a blend, each product-specific component distillery established by reviewed evidence belongs in `proposedBottle.distillers`; do not demote those producing distilleries to `observation` merely because multiple components contribute to the Bottle.",
    "`bottler` is an independently meaningful, market-facing bottler or release imprint for this product. An ordinary official Brand or distillery bottling does not gain a bottler merely because the same company produces, owns, releases, or hosts a page for it. It may equal the Brand or a producing distillery only when product-specific marketing establishes that separate role. Ownership, importer/distributor, and physical packing relationships alone do not establish it. Otherwise leave it null.",
    "Use local Bottle and Entity context first, then focused web evidence for disputed, missing, or identity-critical facts.",
    "For a distinctive, researchable product, make one focused web investigation before finalizing when its combined source and selected Bottle record still lacks core facts: ABV, a product-specific release or bottling year, or distilleries. Use only supported facts in a complete creation or sparse `update_bottle`; otherwise leave them unknown. This is not a general audit or exhaustive search.",
...
const BOTTLE_OPERATION_REVIEW_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "When an evidenced `update_bottle` assigns an existing Entity as Brand, bottler, or distiller, canonical Bottle execution adds that missing role when needed. Do not also propose `update_entity` solely to add the same relationship role.",
...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "`bottler`: independently meaningful market-facing bottler or release imprint for the product; ordinary official Brand or distillery bottlings remain null unless product-specific marketing establishes a separate bottler role.",
...
const BOTTLE_AUDIT_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "Determine the authoritative marketed Bottle identity from the reference, extracted label identity, inspected current Bottle, local candidates, and focused web evidence before proposing repairs, including supported gaps in ABV, release or vintage year, and distilleries.",
    "Do not treat a null bottler as a generic audit gap. Propose one only when product-specific evidence establishes an independently meaningful market-facing bottler or release imprint; ordinary official Brand or distillery bottlings remain null.",
