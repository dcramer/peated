...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "Do not add age, vintage, year, cask, or batch facts to `proposedBottle` just because web results mention them. Use web evidence to confirm the canonical product, but keep `proposedBottle` scoped to source-supported bottle identity.",
  ...
  renderBulletLines([
    ...
    "`repair_bottle`: choose this when an existing bottle is the right identity but its bottle-level canonical fields need correction. Return `matchedBottleId` and the repaired `proposedBottle`.",
    ...
    "For `identityScope = exact_cask`, put source-stated age, ABV, vintage year, cask-strength, and single-cask flags on `proposedBottle` because the exact bottling is the bottle identity; include a source-marketed age in `proposedBottle.name`.",
