...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "A source omission is not a trait conflict, and a candidate's missing enrichment is not evidence of a different Bottle. Do not reject an otherwise exact Bottle match merely because the candidate has a release year the source does not show or because the candidate is missing source-supported ABV or producer enrichment; match it and, after target inspection, propose a sparse update when the missing field is worth repairing. When the source brand exactly matches the inspected candidate Brand entity's alias or short name, do not treat its different canonical Entity name as a Bottle identity conflict when the exact bottle name and all other source-stated identity traits agree.",
  ...
  renderBulletLines([
    ...
    "5. Match only a Bottle candidate that covers the complete observed identity with no unresolved conflicting canonical traits. Return its `matchedBottleId`. Matching an exact existing Bottle is preferred over creating a duplicate. Before rejecting an otherwise uniquely matching candidate because one or more stored fields may be catalog errors, inspect that Bottle. When authoritative exact-product evidence establishes an isolated stored canonical error, match that Bottle and propose a separate sparse `update_bottle`; do not create a duplicate to avoid the bad field. An exact candidate full name can cover a marketed marker even when its structured edition/year fields are sparse, but a shorter marker or prefix is broader identity: for example, a candidate covering only one part of a multi-part edition cannot match the fuller edition. When an observed code is only a lot code, match the plain marketed product row and carry the code in `observation`.",
  ...
  renderBulletLines([
    ...
    "`proposedBottle.name` is the expression relative to the brand and must not normalize to the same text as `proposedBottle.brand.name`. Preserve generic-looking wording when reliable evidence establishes it as a recurring marketed expression. Otherwise, when a product is marketed only by its brand, use a source-supported stable age for the bottle name; use a generic category/style descriptor only when no stable age is supported. If no distinct bottle identity is supported, return `no_match` instead of repeating the brand.",
