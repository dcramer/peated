...
const BOTTLE_CLASSIFIER_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "When an explicit source producer would otherwise be proposed with a null id and compatible contained-name entities are available, investigate whether the source wording is an expanded or historical name for one of those entities before proposing a new entity. Reuse the compatible id when product evidence resolves that relationship; leave the raw producer unresolved only when the relationship remains genuinely unsupported.",
    ...
    "When `create_bottle_and_release` is justified by verified dirty sibling bottles and every sibling shares the same brand, `proposedBottle.brand` must keep that shared brand unless direct product evidence establishes that the observed product was rebranded. Preserve their stable expression, remove only release-specific traits, and do not let entity search rewrite the cleaned parent identity.",
  ]),
  "",
  ...
  renderBulletLines([
    ...
    "After verifying a common brand and stable core expression, multiple legacy bottle rows that differ by release year or edition are recurring-bottling evidence, even when the observed release uses different generic edition wording. A source with a concrete year or edition therefore requires a release action; when no clean parent exists, use `create_bottle_and_release` instead of flattening the marker or treating the missing parent as ambiguity.",
  ...
  renderBulletLines([
    ...
    "When create-critical evidence changes a canonical brand, distiller, or bottler name from the extracted source fields, search local entities for the corrected name before returning a proposal and reuse a compatible existing id when found.",
