...
const BOTTLE_REFERENCE_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "2. Compare local candidates only after step 1. Inspect a plausible target before you select it or reject it for a possible stored error. When the source omits an exact marker, accept a candidate without that marker only when evidence establishes an independently marketed Bottle that covers every supported source trait. Never infer or borrow missing traits from sibling Bottles.",
    ...
    "7. Return `create_bottle` only when no inspected local candidate represents the complete supported identity.",
