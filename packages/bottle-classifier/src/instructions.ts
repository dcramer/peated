...
const BOTTLE_EVIDENCE_POLICY = [
  ...
  renderBulletLines([
    ...
    "Structured extraction is evidence, not authority. Use raw label text or focused external evidence to corroborate uncertain numeric or coded characters before suggesting a catalog change.",
    "A missing source field or missing candidate enrichment is not a conflict. A stored Brand, category, age, or other field can be wrong. Inspect a plausible candidate and use authoritative exact-product evidence before rejecting it or suggesting a change.",
...
const BOTTLE_REFERENCE_INSTRUCTIONS = [
  ...
  renderBulletLines([
    ...
    "2. Compare local candidates. Inspect a plausible target before you select it, reject it for a possible stored error, or suggest a change to it.",
    ...
    "5. Return `match` only when that exact candidate is safe for the reference assignment before any catalog change. An optional sparse Suggested Change can correct a field that does not affect assignment safety.",
    "6. When a candidate needs a catalog change before assignment is safe, record a Suggested Change and return `no_match`. Do not return `match` for the future corrected state.",
  ...
  renderBulletLines([
    ...
    "Use Suggested Change tools only for supported catalog defects that you find while you resolve this Bottle Reference.",
  ]),
  "</tool_policy>",
  "",
