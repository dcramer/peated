export const BOTTLE_SCHEMA_RULES = {
  ...
  exactBottleIdentity:
    "Edition, ABV, vintage year, bottling/release year, exact age, single-cask, cask-strength, marketed finish wording, and exact cask codes can distinguish one Bottle from related Bottles. Preserve explicit cask type, size, and fill values as optional metadata, but never use those three fields alone to select, reject, create, or repair a Bottle. BottleGroup assignment is automatic downstream and is never selected by the classifier.",
  yearPolicy:
