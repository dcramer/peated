export const BOTTLE_SCHEMA_RULES = {
  ...
  exactBottleIdentity:
    "Edition, ABV, vintage year, marketed release year, exact age, single-cask, cask-strength, marketed finish wording, and exact cask codes can distinguish one Bottle from related Bottles. Preserve the complete human-facing edition descriptor, such as `2022 Edition` or `Batch 24`; generic limited-release wording does not replace the stable expression. A printed batch or lot code is an edition only when evidence shows the producer markets it as a distinct release. Preserve explicit cask type, size, and fill values as optional metadata, but never use those three fields alone to select, reject, create, or suggest a Bottle change. BottleGroup assignment is automatic downstream and is never selected by the classifier.",
  yearPolicy:
